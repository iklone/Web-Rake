<?php
	...
	$results = mysqli_query($link, "select scrapeName, scrapeID, sampleData as data from Scrape where taskID ="."'"."$taskID"."'");

	if(mysqli_num_rows($results) >= 1){
		$scrapeList = [];
		while($scrapeInfo = mysqli_fetch_assoc($results)) {
			$id = $scrapeInfo['scrapeID'];
			$values = mysqli_query($link, "SELECT b.scrapeName, b.scrapeID, a.resultValue as data FROM Result a left join Scrape b on a.scrapeID = b.scrapeID WHERE a.ScrapeID = "."'"."$id"."'"." order by resultTime DESC");
